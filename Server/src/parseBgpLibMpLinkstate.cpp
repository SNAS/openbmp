...
//                    parseNlriPrefix(data, nlri_len, id, proto_id, false);
    ...
    void MPLinkState::parseNlriNode(u_char *data, int data_len, uint64_t id, uint8_t proto_id) {
        ...
        SELF_DEBUG("bgp-ls: ID = %x Protocol = %s", id, node_tbl.protocol);
    ...
    void MPLinkState::parseNlriLink(u_char *data, int data_len, uint64_t id, uint8_t proto_id) {
        ...
        SELF_DEBUG("bgp-ls: ID = %x Protocol = %s", id, link_tbl.protocol);
    ...
    void MPLinkState::parseNlriPrefix(u_char *data, int data_len, uint64_t id, uint8_t proto_id, bool isIPv4) {
        ...
        SELF_DEBUG("bgp-ls: ID = %x Protocol = %s", id, prefix_tbl.protocol);
    ...
    int MPLinkState::parseDescrLocalRemoteNode(u_char *data, int data_len, node_descriptor &info) {
        ...
        switch (type) {
            case NODE_DESCR_AS:
            {
                ...
                parse_bgp_lib::SWAP_BYTES(&info.asn);
                data_read += 4;

                SELF_DEBUG("bgp-ls: Node descriptor AS = %u", info.asn);
            ...
            case NODE_DESCR_BGP_LS_ID:
            {
                ...
                parse_bgp_lib::SWAP_BYTES(&info.bgp_ls_id);
                data_read += 4;

                SELF_DEBUG("bgp-ls: Node descriptor BGP-LS ID = %08X", info.bgp_ls_id);
                break;
            }
    ...
    int MPLinkState::parseDescrLink(u_char *data, int data_len, link_descriptor &info) {
        ...
        switch (type) {
            case LINK_DESCR_ID:
            {
                ...
                memcpy(&info.local_id, data, 4); parse_bgp_lib::SWAP_BYTES(&info.local_id);
                memcpy(&info.remote_id, data+4, 4); parse_bgp_lib::SWAP_BYTES(&info.remote_id);
                data_read += 8;

                SELF_DEBUG("bgp-ls: Link descriptor ID local = %08x remote = %08x", info.local_id, info.remote_id);
            ...
            case LINK_DESCR_MT_ID:
            {
                ...
                memcpy(&info.mt_id, data, len); parse_bgp_lib::SWAP_BYTES(&info.mt_id);
                info.mt_id >>= 16;          // MT ID is 16 bits
                data_read += len;

                SELF_DEBUG("bgp-ls: Link descriptor MT-ID = %08x ", info.mt_id);
    ...
    int MPLinkState::parseDescrPrefix(u_char *data, int data_len, prefix_descriptor &info, bool isIPv4) {
        ...
        switch (type) {
            case PREFIX_DESCR_IP_REACH_INFO: {
                ...
                SELF_DEBUG("bgp-ls: prefix ip_reach_info: prefix = %s/%d", ip_char, info.prefix_len);
                break;
            }
            ...
            case PREFIX_DESCR_OSPF_ROUTE_TYPE: {
                ...
                SELF_DEBUG("bgp-ls: prefix ospf route type is %s", info.ospf_route_type);
                break;
            }
